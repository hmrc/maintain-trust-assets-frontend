...
class FakeDraftIdRetrievalActionProvider(
  ...
  when(mockedRepository.get()(any())).thenReturn(Future.successful(dataToReturn))

  override def apply() = new DraftIdDataRetrievalAction(mockedRepository, executionContext)
