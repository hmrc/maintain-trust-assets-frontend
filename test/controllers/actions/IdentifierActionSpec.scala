  ...
  "invoking an AuthenticatedIdentifier" when {
    ...
    "an Agent user hasn't enrolled an Agent Services Account" must {
      ...
      "redirect the user to the create agent services page" in {
        ...
        when(mockAuthService.authenticateAgent()(any(), any()))
    ...
    "the user hasn't logged in" must {
      "redirect the user to log in " in {
        ...
        redirectLocation(
          result
        ).get mustBe "http://localhost:9949/auth-login-stub/gg-sign-in?continue=http%3A%2F%2Flocalhost%3A9781%2Ftrusts-registration&origin=maintain-trust-assets-frontend"
        application.stop()
      }
    }
