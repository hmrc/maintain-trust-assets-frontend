...
import views.html.asset.nonTaxableToTaxable.MaxedOutView
...
class MaxedOutViewSpec extends OptionsViewBehaviours with TabularDataViewBehaviours {
  ...
  "MaxedOutView" when {
    ...
    "taxable" when {
      ...
      val messageKeyPrefix: String = "nonTaxableToTaxable.addAssets"

      val max: Int = 76

      val view: MaxedOutView = viewFor[MaxedOutView](Some(emptyUserAnswers.copy(isMigratingToTaxable = true)))

      def applyView(): HtmlFormat.Appendable =
        view.apply(completeRows(max), "Add assets", max, messageKeyPrefix, isTaxable = true)(fakeRequest, messages)
