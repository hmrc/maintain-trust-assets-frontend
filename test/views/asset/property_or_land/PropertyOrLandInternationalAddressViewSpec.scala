...
class PropertyOrLandInternationalAddressViewSpec extends InternationalAddressViewBehaviours {
  ...
  "PropertyOrLandInternationalAddress view" must {
    ...
    def applyView(form: Form[_]): HtmlFormat.Appendable =
      view.apply(form, index, countryOptions, NormalMode)(fakeRequest, messages)

    behave like pageWithBackLink(applyView(form))

    behave like internationalAddress(applyView, Some(messageKeyPrefix))
