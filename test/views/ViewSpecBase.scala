...
trait ViewSpecBase extends SpecBase {
  ...
  def assertEqualsMessage(doc: Document, cssSelector: String, expectedMessageKey: String, args: Any*): Assertion =
    assertEqualsValue(
      doc,
      cssSelector,
      mockViewUtils.breadcrumbTitle(messages(expectedMessageKey, args: _*))(fakeRequest, messages)
    )
  ...
  def assertPageTitleWithSectionSubheading(doc: Document, expectedMessageKey: String): Assertion = {
    val headers = doc.getElementsByTag("h1")
    headers.size mustBe 1

    val expectedCaption =
      s"${messages(s"$expectedMessageKey.caption.hidden")} ${messages(s"$expectedMessageKey.caption")}"
