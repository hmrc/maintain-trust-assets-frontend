...
final case class UserAnswers(
  ...
  def cleanup: Try[UserAnswers] =
    cleanupBase(
      preserveMoney = false,
      preserveOther = false,
      preservePartnership = false,
      preservePropertyOrLand = false,
      preserveShares = false,
      preserveNonEea = false,
  ...
  private def cleanupBase(
    ...
    val afterOther   =
      afterMoney.flatMap(u => if (preserveOther) Success(u) else u.deleteAtPath(pages.asset.other.basePath))
    val afterPartner =
      afterOther.flatMap(u => if (preservePartnership) Success(u) else u.deleteAtPath(pages.asset.partnership.basePath))
    val afterPoL     = afterPartner.flatMap(u =>
      if (preservePropertyOrLand) Success(u) else u.deleteAtPath(pages.asset.property_or_land.basePath)
    )
    val afterShares  =
      afterPoL.flatMap(u => if (preserveShares) Success(u) else u.deleteAtPath(pages.asset.shares.basePath))
    val afterNonEea  =
      afterShares.flatMap(u => if (preserveNonEea) Success(u) else u.deleteAtPath(pages.asset.noneeabusiness.basePath))
    val afterBiz     =
      afterNonEea.flatMap(u => if (preserveBusiness) Success(u) else u.deleteAtPath(pages.asset.business.basePath))
  ...
  def remove[A](query: Settable[A]): Try[UserAnswers] = {
    ...
    updatedData.flatMap { d =>
      val updatedAnswers = copy(data = d); query.cleanup(None, updatedAnswers)
    }
  }
