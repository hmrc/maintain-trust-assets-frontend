  ...
  def onSubmit(index: Int): Action[AnyContent] = standardActionSets.identifiedUserWithData.async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          if (value) {
            trustService.getPropertyOrLandAsset(request.userAnswers.identifier, index).flatMap {
              _ =>
                trustService.removeAsset(request.userAnswers.identifier, RemoveAsset(AssetNameType.PropertyOrLandAssetNameType, index)).map(_ =>
