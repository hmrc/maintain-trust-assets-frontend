  ...
  override def mapAssets(assets: List[ShareAsset]): List[SharesType] = {
    assets.flatMap {
      case x: ShareNonPortfolioAsset =>
        Some(SharesType(x.quantity, x.name, ShareClass.toDES(x.`class`), x.quoted, x.value, x.startDate))
      case x: SharePortfolioAsset =>
        Some(SharesType(x.quantity, x.name, ShareClass.toDES(ShareClass.Other), x.quoted, x.value, x.startDate))
      case _ =>
        None
    }
  }
