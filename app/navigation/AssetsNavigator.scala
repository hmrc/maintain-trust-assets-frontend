...
class AssetsNavigator @Inject() (config: FrontendAppConfig) {
  ...
  def addAssetRoute(assets: Assets, index: Int): Call = {
    ...
    val routes: List[AssetRoute] = List(
      ...
      AssetRoute(
        assets.propertyOrLand.size,
        MAX_PROPERTY_OR_LAND_ASSETS,
        addAssetNowRoute(PropertyOrLand, assets.propertyOrLand)
      ),
      ...
      AssetRoute(
        assets.nonEEABusiness.size,
        MAX_NON_EEA_BUSINESS_ASSETS,
        addAssetNowRoute(NonEeaBusiness, assets.nonEEABusiness)
      )
    )
  ...
  def addAssetNowRoute(`type`: WhatKindOfAsset, assets: List[AssetType], index: Option[Int] = None): Call =
    `type` match {
      ...
      case NonEeaBusiness =>
        controllers.asset.noneeabusiness.routes.NameController.onPageLoad(index.getOrElse(0), NormalMode)
    }
